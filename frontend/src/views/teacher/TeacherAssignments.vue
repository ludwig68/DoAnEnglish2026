<template>
  <div class="px-10 py-12 min-h-screen bg-slate-50/30 animate__animated animate__fadeIn">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div class="flex items-center gap-3">
          <div class="relative group">
            <i
              class="fa-solid fa-filter absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500 transition-colors"></i>
            ...
            <i
              class="fa-solid fa-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 text-[10px] pointer-events-none"></i>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 mb-10 bg-white border border-slate-50 rounded-[1.5rem] p-1.5 w-fit shadow-sm">
      <button @click="$router.push({ query: { tab: 'queue' } })"
      ...
      <button @click="$router.push({ query: { tab: 'history' } })"
        class="px-8 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 active:scale-95"
    ...
    <template v-if="activeTab === 'queue'">
      <div v-if="isLoading" class="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p class="text-[10px] font-black uppercase tracking-widest text-slate-300 animate-pulse">Đang tải danh sách bài
          nộp...</p>
      </div>
      ...
      <div v-else class="space-y-4">
        <div v-for="sub in queue" :key="sub.id" @click="openGrader(sub.id)"
          ...
          <div class="relative shrink-0">
            <img
              :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(sub.student_name)}&background=f0fdf4&color=10b981&bold=true&size=56`"
              class="w-14 h-14 rounded-2xl shadow-sm group-hover:scale-110 transition-transform duration-500"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-2">
              <span
                class="text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-lg bg-emerald-50 text-emerald-600 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                {{ sub.class_name }}
              </span>
              ...
              <span v-if="sub.deadline && new Date(sub.submitted_at) > new Date(sub.deadline)"
                class="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-red-100 text-red-600 animate-pulse">
                Nộp muộn
              </span>
            </div>
            ...
            <p class="text-xs text-slate-400 font-bold truncate">Bài tập: <span class="text-slate-500 font-black">"{{
              sub.assignment_title }}"</span></p>
          </div>
          ...
          <div class="text-center shrink-0 border-x border-slate-50 px-8">
            ...
            <p class="text-lg font-headline font-black text-slate-700">~{{ sub.approx_word_count }} <span
                class="text-[10px] text-slate-300">TỪ</span></p>
          </div>

          <div class="shrink-0 min-w-[120px] text-right">
            <span v-if="sub.score !== null" class="text-3xl font-headline font-black"
              :class="sub.score >= 7 ? 'text-emerald-500' : sub.score >= 5 ? 'text-amber-500' : 'text-red-400'">
              {{ sub.score.toFixed(1) }}
            </span>
            <div v-else class="flex flex-col items-end">
              <span
                class="px-5 py-2.5 bg-amber-50 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-xl border border-amber-100 group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-500 transition-all">
                Đang chờ chấm
              </span>
            </div>
          </div>
          ...
          <div
            class="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-sm">
    ...
    <template v-else-if="activeTab === 'history'">
      ...
      <div v-else-if="history.length === 0"
        ...
        <p class="text-[13px] mt-1 text-slate-400 font-bold">Lịch sử chấm điểm sẽ xuất hiện tại đây sau khi bạn gửi phản
          hồi.</p>
      </div>

      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div v-for="sub in history" :key="sub.id" @click="openGrader(sub.id)"
          ...
          <img
            :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(sub.student_name)}&background=f1f5f9&color=64748b&bold=true&size=48`"
            class="w-12 h-12 rounded-2xl shrink-0 group-hover:scale-105 transition-transform" :alt="sub.student_name">

          <div class="flex-1 min-w-0">
            <h4
              class="font-headline font-black text-[15px] text-slate-900 truncate leading-none mb-2 group-hover:text-emerald-600 transition-colors">
              {{ sub.student_name }}
            </h4>
          ...
          <div class="text-right shrink-0">
            ...
            <p class="text-[9px] text-slate-300 font-black uppercase tracking-widest">{{ formatDate(sub.submitted_at) }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div v-if="gradingModal"
      class="fixed inset-0 z-[200] flex items-center justify-center p-6 sm:p-10 animate__animated animate__fadeIn">
      <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-md" @click="gradingModal = false"></div>

      <div
        class="bg-white w-full max-w-7xl h-full max-h-[90vh] rounded-[3.5rem] shadow-2xl flex relative overflow-hidden animate__animated animate__zoomIn animate__faster">

        <!-- Cột TRÁI: Nội dung bài luận -->
        <div class="flex-1 flex flex-col overflow-hidden border-r border-slate-50 bg-white">
          <!-- Top Toolbar -->
          <div
            class="px-10 py-8 border-b border-slate-50 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-sm z-10 sticky top-0">
            <div class="flex items-center gap-6">
              <button @click="gradingModal = false"
            ...
            <div class="flex items-center gap-3">
              <button @click="() => window.print()"
                class="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center transition-all hover:shadow-sm">
          ...
          <div class="flex-1 overflow-y-auto px-10 py-10 max-w-4xl mx-auto w-full">
            <div
              class="bg-emerald-50/50 rounded-[2.5rem] p-10 mb-10 border border-emerald-100/50 shadow-sm relative overflow-hidden group">
              <div class="absolute -right-10 -bottom-10 w-32 h-32 bg-emerald-100/30 rounded-full blur-2xl"></div>
              <p
                class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 mb-4 flex items-center gap-2">
            ...
            <div v-if="selectedSub?.submission_content" class="prose prose-slate max-w-none px-4 py-4">
              <div class="text-[17px] leading-[2] text-slate-700 font-medium font-body submission-text"
          ...
          <div class="p-8 border-b border-slate-50">
            <div class="bg-white rounded-[2.5rem] p-6 shadow-sm border border-white flex flex-col gap-6">
              <div class="flex items-center gap-4">
                <img
                  :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(selectedSub?.student_name || '')}&background=10b981&color=fff&bold=true&size=64`"
                  class="w-16 h-16 rounded-2xl shadow-emerald-100 shadow-lg">
                <div class="flex-1 min-w-0">
                  <h4 class="font-headline font-black text-lg text-slate-900 truncate tracking-tight">{{
                    selectedSub?.student_name }}</h4>
                  <p class="text-[11px] text-emerald-600 font-black uppercase tracking-widest mt-1">Lớp: {{
                    selectedSub?.class_name }}</p>
                </div>
              </div>
          ...
          <div class="p-8 flex-1 space-y-10">
            ...
            <div class="space-y-8">
              <div v-for="rubric in rubricItems" :key="rubric.key" class="group">
                ...
                <!-- Custom Slider -->
                <div
                  class="relative h-2.5 bg-white rounded-full border border-slate-100 shadow-sm overflow-hidden group-hover:border-emerald-200 transition-colors">
                  <input type="range" min="0" max="10" step="0.5" v-model.number="rubric.score" :disabled="isGraded"
                    class="absolute inset-0 w-full h-full opacity-0 z-50 transition-all"
                    :class="isGraded ? 'cursor-not-allowed' : 'cursor-pointer'">
                  <div class="h-full rounded-full transition-all duration-300 relative"
            ...
            <!-- Tổng điểm Highlight -->
            <div
              class="bg-white rounded-[3rem] p-8 border border-white shadow-xl shadow-slate-200/50 flex items-center justify-between group">
              <div>
            ...
            <div class="space-y-4">
              <h3
                class="font-headline font-black text-sm text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <i class="fa-solid fa-pen-to-square text-emerald-500"></i> Phản hồi chi tiết
              </h3>
              <textarea v-model="feedbackText" rows="6" :disabled="isGraded"
