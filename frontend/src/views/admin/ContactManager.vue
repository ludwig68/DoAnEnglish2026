    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 flex-1 overflow-hidden flex flex-col">
          ...
          <tbody class="divide-y divide-slate-50 text-sm text-slate-700">
            ...
            >
              ...
              <td class="px-6 py-4 text-slate-500">
                 <p class="line-clamp-2 italic" :title="item.message">"{{ item.message || "Không có nội dung" }}"</p>
              </td>
              <td class="px-6 py-4">
                <span
                  :class="item.is_replied == 1 ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-red-100 text-red-700 border-red-200'"
                  class="px-2.5 py-1 rounded-md text-[0.65rem] font-bold uppercase tracking-wider border inline-flex items-center gap-1"
                >
              ...
              <td class="px-6 py-4 text-slate-500 font-medium">{{ formatDate(item.created_at) }}</td>
      ...
      <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-sm text-slate-500">
        ...
        <span v-if="pendingCount > 0" class="font-bold text-red-500">
          <i class="fa-solid fa-bell mr-1 animate-pulse"></i> Cần xử lý: {{ pendingCount }}
        </span>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate__animated animate__zoomIn animate__faster">
        ...
        <form @submit.prevent="saveContact" class="p-6 space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Họ và tên <span class="text-red-500">*</span></label>
              <input
            ...
            <div>
              <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Email <span class="text-red-500">*</span></label>
            ...
            <div class="md:col-span-2">
              <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Nội dung tin nhắn <span class="text-red-500">*</span></label>
              <textarea
          ...
          <div class="pt-4 border-t border-slate-100">
            <label class="inline-flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700 w-full cursor-pointer hover:bg-slate-100 transition border border-slate-200">
              <input
              ...
              />
              <div>
                 <p class="font-bold" :class="formData.is_replied ? 'text-emerald-600' : 'text-slate-700'">
                   {{ formData.is_replied ? 'Đã xử lý & Phản hồi khách hàng' : 'Đánh dấu là đã phản hồi' }}
                 </p>
                 <p class="text-xs text-slate-500 font-normal">Check vào đây sau khi bạn đã gửi email trả lời khách hàng thành công.</p>
              </div>
            </label>
          </div>
...
const filteredContacts = computed(() => {
  return contacts.value.filter((item) => {
    ...
    // 2. Lọc theo trạng thái is_replied
...
const pendingCount = computed(() => contacts.value.filter(item => item.is_replied == 0).length);
...
// Hàm tự động tạo link Gmail với Tiêu đề và Nội dung soạn sẵn
const getMailLink = () => {
...
const openModal = (mode, item = null) => {
  ...
  if (mode === "edit" && item) {
    formData.value = {
      ...
      is_replied: item.is_replied == 1 ? true : false // Convert TINYINT to Boolean cho UI
    };
  } else {
    resetForm();
  }
...
const validateForm = () => {
  ...
  if (!formData.value.email.includes('@')) {
    alert("Vui lòng nhập email hợp lệ."); return false;
  }
  return true;
};
...
const confirmDelete = async (item) => {
  if (confirm(`Bạn có chắc chắn muốn xóa thư liên hệ của "${item.full_name}" không?`)) {
    try {
